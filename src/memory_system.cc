...
bool MemorySystem::WillAcceptTransaction() const {
    return dram_system_->WillAcceptTransaction();
}
...
bool MemorySystem::AddTransaction(uint64_t hex_addr) {
    return dram_system_->AddTransaction(hex_addr);
}
bool MemorySystem::AddTransaction(uint64_t hex_addr, bool is_write) {
    return dram_system_->AddTransaction(hex_addr, is_write);
}

bool MemorySystem::turnOff() {
    return dram_system_->turn_off;
}
