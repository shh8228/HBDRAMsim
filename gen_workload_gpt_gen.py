...
def gen_workload(n_heads, d_head, start, end, layer, model, mcf):
    ...
    if layer == "all" or "noQKV":
        ...
        gen_actual_workload(folder, "createQKV", M_tile, al, d_model, d_model, 1, mcf, 16/mcf)
        # FC : 1 x d_model x d_model
        # no need

        # Linear 1 : 1 x d_model x 4*d_model
        gen_actual_workload(folder, "L1", M_tile, al, 4*d_model, d_model, 1, mcf, 16/mcf)

        # Linear 2 : 1 x 4*d_model x d_model
        gen_actual_workload(folder, "L2", M_tile, al, d_model, 4*d_model, 1, mcf, 16/mcf)
    if layer != "noQKV":
...
if __name__ == "__main__":
    ...
    for line in lines:
        ...
        for i in [1, 2, 4, 8, 16]:
            gen_workload(int(n_heads), int(d_head), int(args.s), int(args.e), args.l, model, i)
